                ...
                {content.items?.map((item, index) => (
                    <div key={index}
                        ...
                        <TextSlide delay={2000}
                                   className={'font-serif mt-[20px] xl:mt-[50px] !w-fit text-[32px] p-[16px] text-center w-fit xl:h-[130px] xl:w-[80%] mx-auto'}>{t(item.title)}</TextSlide>
                        <div className={'h-[2px] xl:w-[300px] w-full bg-brown-main mb-[16px] mx-auto'}/>
                        <TextSlide delay={2500}
                                   className={'text-[22px] p-[16px] text-center mx-auto h-fit xl:h-[120px]'}>{t(item.blurb)}</TextSlide>
                        <FadeAnimation delay={3} duration={1}
                                       className={'flex place-self-end xl:mt-[20px] mx-auto'}>
                            <TextSlide delay={3000}>
                                <Link to={item.link}
                                      className={'text-brown-main font-semibold hover:font-bold p-[10px] mx-auto w-fit text-[20px] xl:text-[32px] '}>{t(item.linkLabel || 'See more')}</Link>
                            </TextSlide>
                        </FadeAnimation>
                    </div>
                ))}
